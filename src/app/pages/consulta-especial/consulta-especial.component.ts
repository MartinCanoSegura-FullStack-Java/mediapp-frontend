...
export class ConsultaEspecialComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.pacientesFiltrados$ = this.myControlPaciente.valueChanges.pipe(map((val: any) => this.filtrarPacientes(val)));
    this.medicosFiltrados$ = this.myControlMedico.valueChanges.pipe(map((val: any) => this.filtrarMedicos(val)));
